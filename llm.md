...
- `src/ui/update_window.py`: The in-app updater is implemented as a modal `Toplevel` window that performs the download in a separate thread. This keeps the main window visible but inactive, which is a less disruptive user experience than the previous "close-all" approach. Closing the window is handled as a cancellation, which correctly signals the download thread to stop and clean up temporary files.
