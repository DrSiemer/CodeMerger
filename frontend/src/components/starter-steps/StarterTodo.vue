  ...
  <div class="h-full flex flex-col relative" @wheel.ctrl.prevent="handleZoom">
    ...
    <template v-else>
      <div class="max-w-3xl mx-auto w-full space-y-6 text-gray-100 flex flex-col flex-grow min-h-0">
        <h3 class="text-2xl font-bold text-white shrink-0">Generate TODO Plan</h3>
        ...
        <div class="flex justify-between items-center bg-gray-800 p-6 rounded border border-gray-700 shadow-lg shrink-0">
          ...
          <button @click="generateTodo" v-info="'starter_todo_gen'" class="bg-cm-blue hover:bg-blue-500 text-white px-6 py-2.5 rounded shadow transition-colors font-bold">Copy TODO Prompt</button>
        </div>
        ...
        <div v-if="showPasteArea" class="bg-gray-800 p-6 rounded border border-gray-700 shadow-lg space-y-4 flex flex-col flex-grow min-h-0">
          ...
          <textarea v-model="pData.todo_llm_response" v-info="'starter_gen_response'" class="w-full flex-grow min-h-[150px] bg-cm-input-bg border border-gray-600 text-white rounded p-4 outline-none focus:border-cm-blue custom-scrollbar selectable" :style="{ fontSize: editorFontSize + 'px' }" placeholder="Paste response here..."></textarea>
          <div class="flex justify-end shrink-0">
            <button @click="processTodo" v-info="'starter_gen_process'" :disabled="!pData.todo_llm_response" class="bg-cm-green hover:bg-green-600 text-white px-10 py-3 rounded shadow-lg transition-all font-bold disabled:opacity-50 disabled:cursor-not-allowed active:scale-95">Process & Review</button>
          </div>
        </div>
      </div>
    </template>
