...
- `src/ui/window_utils.py`: `position_window` was modified to always prioritize a window's saved position, regardless of which monitor it's on. The constraining logic was also updated to get the monitor from the window's target coordinates rather than the parent's handle, ensuring popups are kept fully visible on their intended screen.
