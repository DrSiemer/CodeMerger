                    ...
                    <div class="flex items-center justify-between p-3">
                      ...
                      <div class="flex items-center space-x-2 shrink-0">
                        ...
                        <template v-else-if="planFileStates[path] === 'pending'">
                          <button @click="toggleDiff(path)" v-info="'review_diff'" class="text-xs font-bold bg-cm-blue hover:bg-blue-500 text-white px-3 py-1 rounded transition-colors flex items-center" title="Inspect text changes">
                            <Eye class="w-3.5 h-3.5 mr-1.5" />
                            Diff
                          </button>
                          <button @click="acceptChange(path, 'modify')" v-info="'review_file_action'" class="text-xs font-bold bg-cm-green hover:bg-green-600 text-white px-3 py-1 rounded transition-colors" title="Apply change to this file">Accept</button>
                          <button @click="discardChange(path)" v-info="'review_file_action'" class="text-xs font-bold bg-gray-700 hover:bg-gray-600 text-gray-300 px-3 py-1 rounded transition-colors" title="Skip this file for now">Discard</button>
                        </template>
                        <button v-else @click="undoChange(path, 'modify')" v-info="'review_file_action'" class="text-xs font-bold bg-gray-800 hover:bg-gray-700 text-gray-400 px-3 py-1 rounded transition-colors flex items-center" title="Revert to local disk version">
                    ...
                    <div class="flex items-center justify-between p-3">
                      ...
                      <div class="flex items-center space-x-2">
                        <template v-if="planFileStates[path] === 'pending'">
                          <button @click="toggleDiff(path)" v-info="'review_diff'" class="text-xs font-bold bg-cm-blue hover:bg-blue-500 text-white px-3 py-1 rounded transition-colors" title="View file content">View</button>
                          <button @click="acceptChange(path, 'create')" v-info="'review_file_action'" class="text-xs font-bold bg-cm-green hover:bg-green-600 text-white px-3 py-1 rounded transition-colors" title="Create this new file">Accept</button>
                          <button @click="discardChange(path)" v-info="'review_file_action'" class="text-xs font-bold bg-gray-700 hover:bg-gray-600 text-gray-300 px-3 py-1 rounded transition-colors" title="Do not create this file">Discard</button>
                        </template>
                        <button v-else @click="undoChange(path, 'create')" v-info="'review_file_action'" class="text-xs font-bold bg-gray-800 hover:bg-gray-700 text-gray-400 px-3 py-1 rounded transition-colors flex items-center" title="Delete the newly created file">
                          <Undo2 class="w-3.5 h-3.5 mr-1.5" />
                          Undo
                        </button>
                      </div>
                ...
                <div v-for="path in lastAiResponse.deletions_proposed" :key="path" class="border border-gray-700 rounded bg-cm-input-bg/30 mb-3">
                  <div class="p-3 flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-2">
                      ...
                      <template v-else-if="planFileStates[path] === 'pending'">
                        <button @click="toggleDiff(path)" v-info="'review_diff'" class="text-xs font-bold bg-cm-blue hover:bg-blue-500 text-white px-3 py-1 rounded transition-colors" title="View file content before deletion">View</button>
                        <button @click="acceptChange(path, 'delete')" v-info="'review_file_action'" class="text-xs font-bold bg-cm-warn hover:bg-red-500 text-white px-3 py-1 rounded transition-colors" title="Delete this file from disk">Accept Delete</button>
                        <button @click="discardChange(path)" v-info="'review_file_action'" class="text-xs font-bold bg-gray-700 hover:bg-gray-600 text-gray-300 px-3 py-1 rounded transition-colors" title="Keep this file">Keep</button>
                      </template>
                      <button v-else @click="undoChange(path, 'delete')" v-info="'review_file_action'" class="text-xs font-bold bg-gray-800 hover:bg-gray-700 text-gray-400 px-3 py-1 rounded transition-colors flex items-center" title="Restore the deleted file">
                        <Undo2 class="w-3.5 h-3.5 mr-1.5" />
                        Undo
                      </button>
                    </div>
