...
<template>
  <transition name="feedback-slide">
    <div v-if="feedback.active" class="absolute inset-0 z-50 p-1.5 flex flex-col" @click.self="$emit('cancel')">
      ...
      <div v-if="feedback.mode !== 'confirm'" class="w-full h-full rounded shadow-lg flex items-center justify-center px-2 py-1 overflow-hidden cursor-pointer" :class="feedback.mode === 'success' ? (feedback.type === 'copy-only' ? 'bg-gray-300' : (feedback.type === 'copy-path' ? 'bg-[#5D4037]' : 'bg-cm-green')) : 'bg-cm-warn'" @click="$emit('cancel')">
