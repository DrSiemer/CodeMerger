...
- **Centralized Prompt Repository (CRITICAL)**: ALL prompt strings, instructions, and template fragments used by the application MUST be defined in the `src/core/prompts/` directory. No prompt text should be hardcoded directly inside API methods or frontend components. This ensures a single source of truth for AI behavior and simplifies prompt engineering across the project.
...
- **Compact Mode Transient State & Multi-Monitor Placement**: Coordinates for the Compact Mode window are strictly transient and reset to `None` on startup. If `None`, the window centers on the main dashboard using Physical Pixel arithmetic. Once placed, the compact window remembers its position even if dragged to another monitor. It will *only* reset its position (re-centering behind the main window) if the main window changes monitors AND moves to a screen different from where the compact window currently sits.
