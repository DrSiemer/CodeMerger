...
"""
    ...
    def _open_rewrite_dialog(self):
        # Save current content first so it's included in the prompt state
        self.segments_data[self.active_key] = self.editor.get("1.0", "end-1c").strip()

        context_data = {
            'keys': self.segment_keys,
            'names': self.friendly_names_map,
    ...
    def _apply_sync_results(self, llm_output):
        ...
        for key, content in mapped.items():
            ...
            if key in self.segments_data and not self.signoff_vars[key].get():
                # If we are rewriting the CURRENT active segment, verify we update the UI
                ...
                if is_active:
                    self.editor.delete("1.0", "end")
                    self.editor.insert("1.0", content)
                    # Don't show "updated" dot for the one we are looking at
