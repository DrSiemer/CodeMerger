...
const copyCorrectionPrompt = async () => {
  if (window.pywebview) {
    const result = await window.pywebview.api.copy_order_admonishment(props.message)
...
<template>
  <div id="order-error-modal" class="absolute inset-0 bg-black/90 flex items-center justify-center z-[100] p-6">
    <div class="bg-cm-dark-bg w-full max-w-[700px] rounded shadow-2xl border border-gray-600 flex flex-col overflow-hidden">
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-700 bg-cm-top-bar">
        <div class="flex items-center text-cm-warn space-x-2">
          ...
          <h2 class="text-lg font-bold">Merge Order Error</h2>
        </div>
      ...
      <div class="p-6">
        ...
        <p class="mt-4 text-gray-500 text-xs italic">
          The response from the LLM is invalid or could not be parsed. Copy the correction prompt below to ask the model to re-output the list correctly.
        </p>
      </div>
