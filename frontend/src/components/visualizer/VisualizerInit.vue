...
import { Network, Copy, Check, AlertTriangle, RefreshCw, Plus, RotateCcw } from 'lucide-vue-next'
...
const emit = defineEmits(['copy-prompt', 'parse', 'cancel', 'copy-correction', 'copy-amend'])

const promptResponse = ref('')
const isPromptCopied = ref(false)
const errorBlockRef = ref(null)

// interactionMode: 'default' | 'error' | 'correct_amend' | 'correct_full'
const interactionMode = ref('default')

watch(() => props.parseError, (newVal) => {
  if (newVal) {
    interactionMode.value = 'error'
    nextTick(() => {
      setTimeout(() => {
...
const startAmend = () => {
  ...
  interactionMode.value = 'correct_amend'
}
...
const pasteLabel = computed(() => {
  if (interactionMode.value === 'correct_amend') return 'Paste Amendment JSON'
  return `2. Paste ${props.mode === 'updating' ? 'Updated ' : ''}LLM Response (JSON)`
})

const pastePlaceholder = computed(() => {
  if (interactionMode.value === 'correct_amend') return 'Paste the { "amendments": ... } JSON here...'
  return `Paste the ${props.mode === 'updating' ? 'updated ' : ''}JSON response here...`
})
</script>
    ...
    <div class="w-full space-y-6 bg-gray-800/50 p-8 rounded-xl border border-gray-700">
      ...
      <div v-if="parseError" ref="errorBlockRef" class="w-full bg-red-900/30 border border-red-700 p-4 rounded-xl space-y-3">
        ...
        <div v-if="interactionMode === 'error'" class="flex space-x-2">
          ...
          <button
            @click="startFullCorrection"
            class="flex-1 bg-red-700 hover:bg-red-600 text-white font-bold py-2 px-4 rounded text-sm transition-colors flex items-center justify-center shadow-lg"
            title="Request the entire list again"
          >
            <Copy class="w-4 h-4 mr-2" /> Full Correction
          </button>
        </div>
      </div>
      ...
      <template v-if="interactionMode !== 'error'">
        ...
        <div class="flex space-x-3">
          <button
            v-if="interactionMode !== 'default'"
            @click="cancelCorrection"
            class="w-1/3 py-3 bg-gray-600 hover:bg-gray-500 text-white font-bold rounded shadow transition-all flex items-center justify-center"
          >
            <RotateCcw class="w-4 h-4 mr-2" />
            Back
          </button>
          <button
            v-else-if="mode === 'updating'"
            @click="emit('cancel')"
            class="w-1/3 py-3 bg-gray-800 hover:bg-gray-700 text-white font-bold rounded shadow transition-all"
          >
            Cancel
          </button>
          ...
          >
            <Network v-if="mode === 'init' && interactionMode !== 'correct_amend'" class="w-4 h-4" />
            <RefreshCw v-else class="w-4 h-4" />
            <span>{{ interactionMode === 'correct_amend' ? 'Apply Amendment' : (mode === 'init' ? 'Visualize Hierarchy' : 'Update Map') }}</span>
          </button>
        </div>
      </template>
    </div>
