...
- **Target Monitor Anchor (Launch Strategy)**: To prevent window "jumps" in multi-monitor setups (where the splash appears on one screen and the app maximizes on another), the `WindowManager` identifies the target monitor handle based on persisted coordinates during the constructor. It then explicitly centers the splash screen on that target monitor and "pins" the main window via `move()` and `resize()` immediately before calling `show()`. This forces the OS window manager to anchor the initialization to the correct workspace.
- **Transient Dimension Reset**: To ensure a consistent workspace on launch, CodeMerger deliberately avoids persisting the main window's width and height to disk. While these values are tracked in memory during a session to support flicker-free transitions (e.g., between Compact and Normal modes), they are excluded from the `main_window_geom` configuration during shutdown. On every application start, the dashboard returns to its default dimensions (1200x780).
