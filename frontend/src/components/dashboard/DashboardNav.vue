...
const prevProfile = async () => {
  ...
  const idx = profiles.findIndex(p => p.id === activeProject.activeProfile)
  const prevIdx = (idx - 1 + profiles.length) % profiles.length
  await switchProfile(profiles[prevIdx].id)
}
...
const nextProfile = async () => {
  ...
  await switchProfile(profiles[nextIdx].id)
}
    ...
    <div class="flex items-center space-x-4">
      ...
      <div id="profile-navigator" class="flex items-center space-x-2" v-if="activeProject.path">
        ...
        <div v-if="activeProject.profiles.length > 1" class="flex items-center" v-info="'profile_nav'">
          ...
          <span class="text-white bg-cm-input-bg px-4 py-2 text-sm font-medium border-y border-gray-600 h-[38px] flex items-center min-w-[80px] justify-center truncate">{{ currentProfileDisplay }}</span>
        ...
        <!-- Single profile view -->
        <span v-else class="text-white bg-cm-input-bg px-4 py-2 rounded text-sm font-medium border border-gray-600 h-[38px] flex items-center truncate max-w-[120px]">{{ currentProfileDisplay }}</span>
