...
const pathClass = computed(() => {
  if (isFailed.value) return 'text-[#DF2622] font-bold'
...
<template>
  <div
    class="border rounded bg-cm-input-bg/30 overflow-hidden transition-colors"
    :class="isFailed ? 'border-red-900/50 bg-red-900/5' : 'border-gray-700'"
  >
    <!-- Header -->
    <div class="flex items-center justify-between p-3" :id="headerId">
      <div class="flex flex-col min-w-0 mr-4">
        ...
        <!-- Error Message positioned below filename as requested -->
        <div v-if="isFailed" class="mt-1.5 flex items-center space-x-2">
            <span class="bg-red-500/10 text-red-500 text-[9px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded border border-red-500/20">Apply Error</span>
            <span class="text-xs text-red-400/80 font-medium italic truncate">{{ errorMessage }}</span>
        </div>
      </div>

      <div class="flex items-center space-x-2 shrink-0">
        <!-- Error State Actions (None) -->
        <div v-if="isFailed"></div>

        <!-- Handled / Skipped State -->
        <div v-else-if="state === 'skipped'">
        ...
        <!-- Pending State -->
        <div v-else-if="state === 'pending'" class="flex items-center space-x-2">
          <button
    ...
    <div v-if="isExpanded" class="p-3 pt-0">
      ...
      <div class="flex justify-end items-center space-x-2 mt-3">
        ...
        <template v-else-if="state !== 'skipped' && state !== 'failed'">
          <button
            @click="$emit('undo', path, type)"
