...
- **Customizable Prompts**: Configure a default prompt that is automatically prepended to your code when using the "Copy Prompt Only" button. You can also set application-wide default intro and outro texts for the instructions feature
...
- **Compact Mode**: A small, always-on-top, draggable window for quick access to core functions that appears when the main window is minimized. It includes an adaptive copy button (Copy with Instructions / Copy Prompt Only) and a paste button.
...
- **Copy Prompt**
    - Click "Copy Prompt Only" to merge the selected files and prepend your custom prompt (configured in Settings). This is useful for providing ongoing context to an LLM. The keyboard shortcut for this action is **`Ctrl+Shift+C`**.
...
- **Compact Mode**
    ...
    - The panel contains two primary buttons:
        - **Copy Prompt (Instructions/Only)**: A single adaptive button for copying code.
            - It appears as "Copy with Instructions" if you have defined instructions for the project, and "Copy Prompt Only" if you have not.
            - A normal click performs the action shown on the button.
            - Holding **Ctrl** while clicking will always perform the "Copy Prompt Only" action.
...
- **Prompts**:
    - **"Copy Prompt Only" Prompt**: Set the default text that is automatically prepended when you click "Copy Prompt Only"
