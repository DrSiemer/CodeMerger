...
class Step2ConceptView(tk.Frame):
    ...
    def show_initial_view(self):
        ...
        if existing_goal:
            ...
            prefix = "I want to build...\n\n"
            self.goal_text.insert("1.0", f"{prefix}{description}")

        RoundedButton(self, text="Generate Concept Prompt", command=self.handle_prompt_generation, bg=c.BTN_BLUE, fg=c.BTN_BLUE_TEXT, font=c.FONT_BUTTON, height=30, cursor="hand2").grid(row=3, column=0, pady=10, sticky="e")
        tk.Frame(self, bg=c.DARK_BG).grid(row=4, column=0)

    def handle_prompt_generation(self):
        user_goal = self.goal_text.get("1.0", 'end-1c')
        if not user_goal.strip() or user_goal.strip() == "I want to build...":
