    ...
    <template v-else>
      <div class="max-w-3xl mx-auto w-full space-y-6 text-gray-100">
        ...
        <div v-if="showPasteArea" class="bg-gray-800 p-4 rounded border border-gray-700 mt-4">
          ...
          <textarea v-model="pData.design_llm_response" class="w-full h-40 bg-cm-input-bg border border-gray-600 text-white rounded p-4 outline-none focus:border-cm-blue custom-scrollbar selectable" :style="{ fontSize: editorFontSize + 'px' }" placeholder="Paste response here..."></textarea>
          <div class="flex justify-end mt-3">
            <button @click="processDesign" :disabled="!pData.design_llm_response" class="bg-cm-green hover:bg-green-600 text-white px-6 py-2 rounded shadow transition-colors disabled:opacity-50 font-bold">Process & Review</button>
          </div>
        </div>
      </div>
    </template>
